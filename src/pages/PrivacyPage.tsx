  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-6 pt-32 pb-16">
        <h1 className="text-4xl font-bold text-text-primary mb-2">{t('privacy.title')}</h1>
        <p className="text-text-secondary text-sm mb-8">{t('privacy.lastUpdated')}</p>

        <div className="border border-border rounded-2xl overflow-hidden">
          {sections.map(({ title, content }) => (
            <section key={title} className="px-12 py-9">
